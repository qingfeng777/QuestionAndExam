  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12 page-content">
        ...
        <p>查看待参加的考试信息</p>
        <div class="page_nav">
      <span>当前位置:</span> <a href="index.html">测试</a> <span><i class="fa fa-angle-double-right"></i>待参加的考试</span>
      </div>
      </div>
    </div>
  </div>
...
<div style="height:450px;">
	<form action="tostudentTestInfo" method="get" style="margin-top:80px;width: 80%;margin-left: 5.5%;">
	    <input name="search" type="search" value="" placeholder="搜索...."  required/>
	    <button type="submit" class="btn btn_common blue">查询</button>
	  </form>
	  <br>
<table class="table table-hover" style="margin-top:80px;width: 80%;margin-left: 3.5%;">
	...
	<c:forEach items="${requestScope.testPaperList }" var="testPaper">
		<tr>
			<td>${testPaper.testPaperInfoId }</td>
			<td>${testPaper.subjectId }</td>
			<td>${testPaper.teacherId }</td>
			<td>${testPaper.beginTime}</td>
			<td>${testPaper.endTime }</td>
			<td>${testPaper.location}</td>
			<td>${testPaper.proctor}</td>
			...
			<td><a href="attendTest?paperId=${testPaper.testPaperInfoId }">
				<input class="btn btn-default btn-xs" type="button" name="button" value="参加考试">
				</a>
			</td>
		</tr>
	</c:forEach>
...
<!-- 		<input class="btn btn-default" type="button" value="编辑信息" onclick="edit()">
 -->		<a href="toPaperAdd"><input class="btn btn-default" type="button" value="添加" ></a>
	</div>
<!-- ***********************************页脚************************************** -->
